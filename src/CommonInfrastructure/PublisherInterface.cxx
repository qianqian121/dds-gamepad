...
// ------------------------------------------------------------------------- //
template <class T>
PublisherInterface::PublisherInterface(std::vector<std::string> xmlFiles) 
{
	...
	// Downcast the generic datawriter to a DataWriter 
	_writer = DataWriter::narrow(writer);
...
template <class T>
bool PublisherInterface::Write(DdsAutoType<T> data)
{
...
template <class T>
bool PublisherInterface::Delete(DdsAutoType<T> data)
{
	DDS_ReturnCode_t retcode = DDS_RETCODE_OK;
	DDS_InstanceHandle_t handle = DDS_HANDLE_NIL;
